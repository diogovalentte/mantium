...
function ChapterPages(chapterURL)
    ...
    doc:find("script"):each(function(_, el)
        ...
        local js = el:text()
        if not contains(js, "decode_images_100") then
            return nil
        end
        found = true

        local chapter_id = extractChapterID(js)
        if chapter_id == "" then
            return {}
        end

        local number = extractNumber(js)
        if number == "" then
            return {}
        end

        local req_url = Base .. "/wp-admin/admin-ajax.php"
        local page_idx = 1
        local img_index = 0
        while true do
            ...
            local request = Http.request("POST", req_url, data)
