...
- The API waits until the Kaizoku job queues are empty to add jobs to download new chapters. But as it can take a long time, the API will timeout after some minutes while waiting. You can set the minutes the API will wait using an environment variable.
