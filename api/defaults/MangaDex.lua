...
function MangaChapters(mangaURL)
    ...
    local occurence = {}
    for _, val in pairs(result_body["data"]) do
        local chapterChapter = val["attributes"]["chapter"]
        if not occurence[chapterChapter] then
            occurence[chapterChapter] = true
...
function ChapterPages(chapterURL)
    ...
    for _, val in pairs(result_body["chapter"]["data"]) do
        local page =
        { index = i, url = result_body["baseUrl"] .. "/data/" .. result_body["chapter"]["hash"] .. "/" .. val }
        pages[i] = page
        i = i + 1
    end
